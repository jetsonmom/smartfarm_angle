...
const byte PLANT_LED_PIN = 6;        // 식물용 LED (오전 5시~저녁 23시)
const byte EXTERNAL_FAN_PIN = 7;     // 외부 쿨링팬 (워터펌프 작동시 함께 작동)
...
#define EXTERNAL_FAN_ON_BIT 3
#define DEBUG_MODE_BIT 4

// LED 작동 시간 설정
const byte LED_ON_HOUR = 5;   // 오전 5시 켜짐
const byte LED_OFF_HOUR = 21; // 저녁 23시 꺼짐

// 펌프 타이머
unsigned long pumpStartTime = 0;
const unsigned int PUMP_DURATION = 60000; // 1분
const unsigned int PUMP_MAX_DURATION = 120000; // 2분
...
bool externalFanIsOn() { return CHECK_BIT(deviceStatus, EXTERNAL_FAN_ON_BIT); }
...
void setExternalFanOn(bool on) { on ? SET_BIT(deviceStatus, EXTERNAL_FAN_ON_BIT) : CLEAR_BIT(deviceStatus, EXTERNAL_FAN_ON_BIT); }
...
void setup() {
  ...
  // 모든 릴레이 초기화
  // 3번과 5번 핀은 LOW로 꺼짐, 나머지는 HIGH로 꺼짐
  digitalWrite(INTERNAL_FAN_PIN, LOW);     // 3번 핀 - 꺼짐 (LOW = 꺼짐)
  digitalWrite(GROWTH_PUMP_PIN, HIGH);     // 4번 핀 - 꺼짐 (HIGH = 꺼짐)
  digitalWrite(GERMINATION_PUMP_PIN, LOW); // 5번 핀 - 꺼짐 (LOW = 꺼짐)
  digitalWrite(PLANT_LED_PIN, HIGH);       // 6번 핀 - 꺼짐 (HIGH = 꺼짐)
  digitalWrite(EXTERNAL_FAN_PIN, HIGH);    // 7번 핀 - 꺼짐 (HIGH = 꺼짐)
...
void loop() {
  ...
  // 안전 장치: 펌프가 너무 오래 작동하면 강제 중지
  if (pumpIsActive() && (millis() - pumpStartTime >= PUMP_MAX_DURATION)) {
    Serial.println(F("안전: 펌프가 너무 오래 작동 중, 강제 중지"));
    stopPumps();
  }
...
void processInputData() {
  ...
  // 강제로 3번 핀 끄기 (내부 팬)
  else if (strcmp(serialBuffer, "FORCE_FAN_OFF") == 0) {
    digitalWrite(INTERNAL_FAN_PIN, LOW);  // 3번 핀 - LOW로 꺼짐
    setInternalFanOn(false);
    Serial.println(F("내부 팬 강제 종료"));
  }
  // 강제로 5번 핀 끄기 (발아용 펌프)
  else if (strcmp(serialBuffer, "FORCE_GPUMP_OFF") == 0) {
    digitalWrite(GERMINATION_PUMP_PIN, LOW);  // 5번 핀 - LOW로 꺼짐
    Serial.println(F("발아용 펌프 강제 종료"));
  }
}
...
void controlInternalFan() {
  ...
  // 3번 팬이 잘못 켜져 있는지 확인
  if (temperature < TEMP_THRESHOLD && digitalRead(INTERNAL_FAN_PIN) == HIGH) {  // 3번 핀 - HIGH가 켜짐
...
void startPumps() {
  ...
  digitalWrite(GROWTH_PUMP_PIN, LOW);  // 4번 핀 - LOW로 켜짐
  digitalWrite(GERMINATION_PUMP_PIN, HIGH);  // 5번 핀 - HIGH로 켜짐
  digitalWrite(EXTERNAL_FAN_PIN, LOW);  // 7번 핀 - LOW로 켜짐
  
  setPumpActive(true);
  setExternalFanOn(true);
  pumpStartTime = millis();
  
  Serial.println(F("펌프와 외부 팬 시작"));
}

// 펌프 중지
void stopPumps() {
  digitalWrite(GROWTH_PUMP_PIN, HIGH);  // 4번 핀 - HIGH로 꺼짐
  digitalWrite(GERMINATION_PUMP_PIN, LOW);  // 5번 핀 - LOW로 꺼짐
  digitalWrite(EXTERNAL_FAN_PIN, HIGH);  // 7번 핀 - HIGH로 꺼짐
  
  setPumpActive(false);
  setExternalFanOn(false);
  
  Serial.println(F("펌프와 외부 팬 정지"));
}
...
void printDebugInfo() {
  ...
  Serial.print(F("외부 팬: "));
  Serial.println(externalFanIsOn() ? F("켜짐") : F("꺼짐"));
  
  Serial.println(F("--- 핀 상태 ---"));
  Serial.print(F("내부 팬(3): "));
  Serial.println(digitalRead(INTERNAL_FAN_PIN) == HIGH ? F("켜짐") : F("꺼짐"));  // 3번 핀 - HIGH가 켜짐
  
  Serial.print(F("성장 펌프(4): "));
  Serial.println(digitalRead(GROWTH_PUMP_PIN) == LOW ? F("켜짐") : F("꺼짐"));  // 4번 핀 - LOW가 켜짐
  
  Serial.print(F("발아 펌프(5): "));
  Serial.println(digitalRead(GERMINATION_PUMP_PIN) == HIGH ? F("켜짐") : F("꺼짐"));  // 5번 핀 - HIGH가 켜짐
  
  Serial.print(F("LED(6): "));
  Serial.println(digitalRead(PLANT_LED_PIN) == LOW ? F("켜짐") : F("꺼짐"));  // 6번 핀 - LOW가 켜짐
  
  Serial.print(F("외부 팬(7): "));
  Serial.println(digitalRead(EXTERNAL_FAN_PIN) == LOW ? F("켜짐") : F("꺼짐"));  // 7번 핀 - LOW가 켜짐
...
void checkForMalfunctions() {
  ...
  // 5번 핀(발아용 펌프) 확인 - 펌프가 작동 중이 아닌데 켜져 있으면 문제
  if (!pumpIsActive() && digitalRead(GERMINATION_PUMP_PIN) == HIGH) {  // 5번 핀 - HIGH가 켜짐
    Serial.println(F("오작동: 발아 펌프 강제 종료"));
    digitalWrite(GERMINATION_PUMP_PIN, LOW);  // 5번 핀 - LOW로 꺼짐
  }
}
